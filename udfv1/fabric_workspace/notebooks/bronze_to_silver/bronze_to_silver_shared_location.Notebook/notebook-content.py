...
# # Load Bronze Data to Silver Table - Location
# 
# ## Overview
# Load Location sample data from Bronze lakehouse files into Silver lakehouse table.
# 
# ## Data Flow
# - **Source**: Bronze Lakehouse /Files/samples_fabric/shared/Location_Samples.csv
# - **Target**: Silver Lakehouse shared.Location table (Delta table)
...
SOURCE_PATH = f"abfss://{WORKSPACE_ID}@onelake.dfs.fabric.microsoft.com/{SOURCE_LAKEHOUSE_ID}/Files/samples_fabric/shared/Location_Samples.csv"
...
# Required columns from Model_Shared_Data.ipynb Location table
required_columns = [
    "LocationId", "CustomerId", "LocationName", "IsActive", "AddressLine1", "AddressLine2", "City",
    "StateId", "ZipCode", "CountryId", "SubdivisionName", "Region", "Latitude", "Longitude", "Note", "UpdatedBy"
]
...
# Show value distributions for LocationId
print(f"\n🎯 LocationId Distribution:")
df.groupBy("LocationId").count().orderBy("LocationId").show()
...
try:
    ...
    spark.sql(f"SELECT * FROM {TARGET_FULL_PATH} ORDER BY LocationId").show(10, truncate=False)

    print(f"🎉 Location data load complete!")
