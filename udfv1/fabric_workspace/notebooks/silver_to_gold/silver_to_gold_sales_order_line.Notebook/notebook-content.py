...
# # Load Silver Table to Gold Table - OrderLine
# 
# ## Overview
# Load OrderLine data from Silver lakehouse table to Gold lakehouse table.
# 
# ## Data Flow
# - **Source**: MAAG_LH_Silver.sales.OrderLine (Silver lakehouse table)
# - **Target**: MAAG_LH_Gold.sales.OrderLine (Gold lakehouse - attached as default)
...
print(f"🔄 Loading OrderLine from Silver to Gold")
...
duplicate_count = df_gold.groupBy("OrderId", "OrderLineNumber").count().filter(col("count") > 1).count()
if duplicate_count > 0:
    print(f"⚠️ Found {duplicate_count} duplicate OrderId+OrderLineNumber combinations")
else:
...
null_checks = df_gold.select(
    ...
    spark_sum(col("OrderLineNumber").isNull().cast("int")).alias("null_orderlinenumber"),
    spark_sum(col("ProductId").isNull().cast("int")).alias("null_productid")
).collect()[0]

if null_checks["null_orderid"] > 0 or null_checks["null_orderlinenumber"] > 0 or null_checks["null_productid"] > 0:
    print(f"⚠️ Found nulls: OrderId={null_checks['null_orderid']}, OrderLineNumber={null_checks['null_orderlinenumber']}, ProductId={null_checks['null_productid']}")
else:
