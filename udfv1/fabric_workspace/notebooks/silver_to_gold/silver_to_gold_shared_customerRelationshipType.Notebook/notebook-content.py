...
# # Load Silver Table to Gold Table - shared.customerRelationshipType
# 
# ## Overview
# Load CustomerRelationshipType data from Silver lakehouse table to Gold lakehouse table.
# 
# ## Data Flow
# - **Source**: Silver Lakehouse shared.customerRelationshipType
# - **Target**: Gold Lakehouse shared.customerRelationshipType
...
SOURCE_TABLE = "customerrelationshiptype"
...
print(f"🔄 Loading CustomerRelationshipType from Silver to Gold")
...
df_gold = df.select(
    col("CustomerRelationshipTypeId"),
    col("CustomerRelationshipTypeName"), 
    col("CustomerRelationshipTypeDescription"),
...
duplicate_count = df_gold.groupBy("CustomerRelationshipTypeId").count().filter(col("count") > 1).count()
if duplicate_count > 0:
    print(f"⚠️ Found {duplicate_count} duplicate CustomerRelationshipTypeIds")
else:
    print(f"✅ No duplicates found")

# Check for nulls in key fields
null_checks = df_gold.select(
    spark_sum(col("CustomerRelationshipTypeId").isNull().cast("int")).alias("null_ids"),
    spark_sum(col("CustomerRelationshipTypeName").isNull().cast("int")).alias("null_names")
).collect()[0]
