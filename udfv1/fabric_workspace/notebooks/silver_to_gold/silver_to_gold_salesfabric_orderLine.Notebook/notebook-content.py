...
# # Load Silver Table to Gold Table - salesfabric.orderLine
# 
# ## Overview
# Load OrderLine data from Silver lakehouse table to Gold lakehouse table.
# 
# ## Data Flow
# - **Source**: Silver Lakehouse salesfabric.orderLine
# - **Target**: Gold Lakehouse salesfabric.orderLine
...
duplicate_count = (
    df_gold.groupBy("OrderId", "OrderLineNumber")
    .count()
    .filter(col("count") > 1)
    .count()
)
if duplicate_count > 0:
    print(f"⚠️ Found {duplicate_count} duplicate OrderId + OrderLineNumber pairs")
else:
...
null_checks = df_gold.select(
    ...
    spark_sum(col("OrderLineNumber").isNull().cast("int")).alias("null_linenumber")
).collect()[0]

if null_checks["null_orderid"] > 0 or null_checks["null_linenumber"] > 0:
    print(f"⚠️ Found nulls: OrderId={null_checks['null_orderid']}, OrderLineNumber={null_checks['null_linenumber']}")
else:
