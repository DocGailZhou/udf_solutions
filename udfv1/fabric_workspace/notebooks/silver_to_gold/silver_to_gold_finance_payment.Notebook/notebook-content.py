...
# 
# ## Data Flow
# - **Source**: Silver lakehouse.finance.payment (Silver lakehouse table)
# - **Target**: Gold lakehouse.finance.payment (Gold lakehouse - attached as default)
...
print(f"🔄 Loading Payment from Silver to Gold")
...
if duplicate_count > 0:
    print(f"⚠️ Found {duplicate_count} duplicate PaymentId values")
else:
...
if null_checks["null_paymentid"] > 0 or null_checks["null_paymenttype"] > 0:
    print(f"⚠️ Found nulls: PaymentId={null_checks['null_paymentid']}, PaymentType={null_checks['null_paymenttype']}")
else:
