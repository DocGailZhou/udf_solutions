...
# # Load Silver Table to Gold Table - shared.customerTradeName
# 
# ## Overview
# Load CustomerTradeName data from Silver lakehouse table to Gold lakehouse table.
# 
# ## Data Flow
# - **Source**: Silver Lakehouse shared.customerTradeName
# - **Target**: Gold Lakehouse shared.customerTradeName
...
SOURCE_TABLE = "customertradename"
...
print(f"🔄 Loading CustomerTradeName from Silver to Gold")
...
duplicate_count = df_gold.groupBy("TradeNameId").count().filter(col("count") > 1).count()
if duplicate_count > 0:
    print(f"⚠️ Found {duplicate_count} duplicate TradeNameIds")
else:
    print(f"✅ No duplicates found")

# Check for nulls in key fields
null_checks = df_gold.select(
    spark_sum(col("TradeNameId").isNull().cast("int")).alias("null_ids"),
    spark_sum(col("TradeName").isNull().cast("int")).alias("null_names")
).collect()[0]
