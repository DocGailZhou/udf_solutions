...
# # Load Silver Table to Gold Table - shared.customer
# 
# ## Overview
# Load Customer data from Silver lakehouse table to Gold lakehouse table.
# 
# ## Data Flow
# - **Source**: Silver Lakehouse shared.customer
# - **Target**: Gold Lakehouse shared.customer
...
print(f"🔄 Loading Customer from Silver to Gold")
...
duplicate_count = df_gold.groupBy("CustomerId").count().filter(col("count") > 1).count()
if duplicate_count > 0:
    print(f"⚠️ Found {duplicate_count} duplicate CustomerIds")
else:
    print(f"✅ No duplicates found")

# Check for nulls in key fields
null_checks = df_gold.select(
    spark_sum(col("CustomerId").isNull().cast("int")).alias("null_ids"),
    spark_sum(col("FirstName").isNull().cast("int")).alias("null_firstname"),
    spark_sum(col("LastName").isNull().cast("int")).alias("null_lastname")
).collect()[0]

if null_checks["null_ids"] > 0 or null_checks["null_firstname"] > 0 or null_checks["null_lastname"] > 0:
    print(f"⚠️ Found nulls: IDs={null_checks['null_ids']}, FirstName={null_checks['null_firstname']}, LastName={null_checks['null_lastname']}")
else:
