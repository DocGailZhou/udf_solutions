...
# # Load Silver Table to Gold Table - salesfabric.order
# 
# ## Overview
# Load Order data from Silver lakehouse table to Gold lakehouse table.
# 
# ## Data Flow
# - **Source**: Silver Lakehouse salesfabric.order
# - **Target**: Gold Lakehouse salesfabric.order
...
null_checks = df_gold.select(
    spark_sum(col("OrderId").isNull().cast("int")).alias("null_ids"),
    spark_sum(col("OrderNumber").isNull().cast("int")).alias("null_ordernumber")
).collect()[0]

if null_checks["null_ids"] > 0 or null_checks["null_ordernumber"] > 0:
    print(f"⚠️ Found nulls: IDs={null_checks['null_ids']}, OrderNumber={null_checks['null_ordernumber']}")
else:
