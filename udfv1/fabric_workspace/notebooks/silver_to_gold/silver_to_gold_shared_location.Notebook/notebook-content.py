...
# # Load Silver Table to Gold Table - shared.location
# 
# ## Overview
# Load Location data from Silver lakehouse table to Gold lakehouse table.
# 
# ## Data Flow
# - **Source**: Silver Lakehouse shared.location
# - **Target**: Gold Lakehouse shared.location
...
print(f"🔄 Loading Location from Silver to Gold")
...
duplicate_count = df_gold.groupBy("LocationId").count().filter(col("count") > 1).count()
if duplicate_count > 0:
    print(f"⚠️ Found {duplicate_count} duplicate LocationIds")
else:
    print(f"✅ No duplicates found")

# Check for nulls in key fields
null_checks = df_gold.select(
    spark_sum(col("LocationId").isNull().cast("int")).alias("null_ids"),
    spark_sum(col("LocationName").isNull().cast("int")).alias("null_names")
).collect()[0]
